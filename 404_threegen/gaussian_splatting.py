...
def setup_material():
    ...
    res_nodes = [
        scale_node_0,
        #            scale_node_1, scale_node_2, scale_node_3, scale_node_4, scale_node_5, scale_node_6,
        #            scale_node_7,
...
def setup_geonodes(obj, mat):
    ...
    geo_tree.links.new(opacity_attr_gn_node.outputs["Attribute"], threshold_node.inputs[0])
    geo_tree.links.new(threshold_value_node.outputs[0], threshold_node.inputs[1])
    geo_tree.links.new(threshold_node.outputs[0], join_selection_node.inputs[0])
    geo_tree.links.new(maximum_node.outputs["Value"], join_selection_node.inputs[1])
    geo_tree.links.new(join_selection_node.outputs[0], mesh_to_points_node.inputs["Selection"])
    ...
    geo_tree.links.new(instance_node.outputs["Instances"], switch_node.inputs[1])

    geo_tree.links.new(mesh_to_points_node.outputs["Points"], set_point_radius_node.inputs["Points"])
    geo_tree.links.new(set_point_radius_node.outputs["Points"], switch_node.inputs[2])
    geo_tree.links.new(switch_node.outputs[0], set_material_node.inputs["Geometry"])
